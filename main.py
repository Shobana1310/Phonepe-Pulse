...
map_transaction_data.drop(map_transaction_data[map_transaction_data['State'] == 'Dadra And Nagar Haveli And Daman And Diu'].index, inplace=True)
map_insurance_data.drop(map_insurance_data[map_insurance_data['State'] == 'Dadra And Nagar Haveli And Daman And Diu'].index, inplace=True)
map_transaction_data['State']=map_transaction_data['State'].replace('Jammu And Kashmir','Jammu & Kashmir')
  ...
  if Analyse_method=='User':
    with st.container():
           ...
           with left_column:
               st.write('')
               st.write('')
               image_path ="https://i.postimg.cc/cC55rY2H/team.png"
  ...
  if Analyse_method=='Transaction':
        with st.container():
           ...
           with left_column:
               st.write('')
               st.write('')
               image_path ="https://i.postimg.cc/L8vNw1Bh/money-transfer.png"
               st.image(image_path,width=150)
  ...
  if Analyse_method=='Insurance':
        with st.container():
           ...
           with left_column:
               st.write('')
               st.write('')
               image_path ='https://i.postimg.cc/W18Lv9wm/insurance.png'
               st.image(image_path,width=150)
        option=st.radio('How would you like to Analyse?',['State Wise','District Wise'],index=None,horizontal=True)  
        st.write('You selected:', option)
                     ...
                     with tab1:
                        def Districts_EachState_Insurance_low(df,state):
                              ...
                              fig = px.bar(top_10, 
                                          ...
                                          labels={'District': 'Districts', 'Insurance_count': 'Insurance_counts'},
                                          color='District',
                                          color_discrete_sequence=px.colors.qualitative.Dark24,text_auto=True)
                              fig.update_layout(
                                 font=dict(
                                    family="sans-serif,verdana",
                                    size=12,
